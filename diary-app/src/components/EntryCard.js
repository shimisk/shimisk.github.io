...
export default function EntryCard({ entry, themeId, cardIndex, onClick }) {
  ...
  return h('div', {
    ...
    h('div', { className: "card-main" },
      h('div', { className: "card-date-block" },
        h('div', { className: "card-day-number" }, dayNumber),
      ...
      h('div', { className: "card-text" },
        entry.title && h('div', { className: "card-title" }, entry.title),
        h('div', { className: "card-snippet" }, entry.body)
      )
    )
  );
}
