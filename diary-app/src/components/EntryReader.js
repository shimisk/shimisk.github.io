...
export default function EntryReader({ entry, onBack, onEdit, onDelete }) {
        ...
        h('div', { className: "read-sticker-box" },
          entry.sticker
            ? (isImageSticker(entry.sticker)
              ? h('img', { className: "read-sticker-image", src: entry.sticker, alt: stickerLabel(entry.sticker) })
              : h('div', { className: "read-sticker" }, entry.sticker))
